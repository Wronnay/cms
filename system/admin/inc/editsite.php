...
<?php 
if(isset($_GET['id'])){
	$abfrage = mysql_query("SELECT name, title, text, date, description, tags FROM ".$PREFIX."_sites WHERE id='".presql($_GET['id'])."'");
...
mysql_query("UPDATE ".$PREFIX."_sites SET autor_id = '".presql($_SESSION["ADMINid"])."', name = '".presql($_REQUEST['name'])."', title = '".presql($_REQUEST['title'])."', text = '".presql($_REQUEST['text'])."', description = '".presql($_REQUEST['description'])."', tags = '".presql($_REQUEST['keywords'])."' WHERE id = '".presql($_GET['id'])."'");
	  echo w19;
	  }
  }
?>
